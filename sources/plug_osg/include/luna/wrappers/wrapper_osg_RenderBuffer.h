...
public:
	...
	static int _bind_public_dirtyAll(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call void osg::RenderBuffer::public_dirtyAll() const. Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		wrapper_osg_RenderBuffer* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_RenderBuffer >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call osg::RenderBuffer & osg::RenderBuffer::public_op_assign(const osg::RenderBuffer &). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
