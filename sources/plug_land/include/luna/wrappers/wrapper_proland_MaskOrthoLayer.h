#ifndef _WRAPPERS_WRAPPER_PROLAND_MASKORTHOLAYER_H_
#define _WRAPPERS_WRAPPER_PROLAND_MASKORTHOLAYER_H_

#include <plug_common.h>

#include "lua/LuaObject.h"

#include <W:/Cloud/Projects/sgt/sources/proland/sources/graph/proland/ortho/MaskOrthoLayer.h>

class wrapper_proland_MaskOrthoLayer : public proland::MaskOrthoLayer, public luna_wrapper_base {

public:
		

	~wrapper_proland_MaskOrthoLayer() {
		logDEBUG3("Calling delete function for wrapper proland_MaskOrthoLayer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::MaskOrthoLayer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_MaskOrthoLayer(lua_State* L, lua_Table* dum, ork::ptr< proland::GraphProducer > graphs, set< int > ignored, ork::ptr< ork::Program > layerProgram, int writeMask, ork::vec4f color, float depth, float widthFactor, proland::MaskOrthoLayer::BlendParams blendParams, ork::vec4f blendColor, int displayLevel, bool deform = false) 
		: proland::MaskOrthoLayer(graphs, ignored, layerProgram, writeMask, color, depth, widthFactor, blendParams, blendColor, displayLevel, deform), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_proland_MaskOrthoLayer(lua_State* L, lua_Table* dum) 
		: proland::MaskOrthoLayer(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.callFunction<void>();
		}
	};


	// Private virtual methods:
protected:
	// Protected virtual methods:
	// void ork::Object::doRelease()
	void doRelease() {
		if(_obj.pushFunction("doRelease")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::doRelease();
	};

public:
	// Public virtual methods:
	// const char * ork::Object::toString()
	const char * toString() {
		if(_obj.pushFunction("toString")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			return (_obj.callFunction<const char*>());
		}

		return MaskOrthoLayer::toString();
	};

	// void proland::TileLayer::setCache(ork::ptr< proland::TileCache > cache, int producerId)
	void setCache(ork::ptr< proland::TileCache > cache, int producerId) {
		if(_obj.pushFunction("setCache")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg((proland::TileCache*)cache.get());
			_obj.pushArg(producerId);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::setCache(cache, producerId);
	};

	// void proland::TileLayer::invalidateTiles()
	void invalidateTiles() {
		if(_obj.pushFunction("invalidateTiles")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::invalidateTiles();
	};

	// void proland::GraphLayer::getReferencedProducers(vector< ork::ptr< proland::TileProducer > > & producers) const
	void getReferencedProducers(vector< ork::ptr< proland::TileProducer > > & producers) const {
		if(_obj.pushFunction("getReferencedProducers")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(&producers);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::getReferencedProducers(producers);
	};

	// void proland::GraphLayer::useTile(int level, int tx, int ty, unsigned int deadline)
	void useTile(int level, int tx, int ty, unsigned int deadline) {
		if(_obj.pushFunction("useTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(level);
			_obj.pushArg(tx);
			_obj.pushArg(ty);
			_obj.pushArg(deadline);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::useTile(level, tx, ty, deadline);
	};

	// void proland::GraphLayer::unuseTile(int level, int tx, int ty)
	void unuseTile(int level, int tx, int ty) {
		if(_obj.pushFunction("unuseTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(level);
			_obj.pushArg(tx);
			_obj.pushArg(ty);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::unuseTile(level, tx, ty);
	};

	// void proland::GraphLayer::prefetchTile(int level, int tx, int ty)
	void prefetchTile(int level, int tx, int ty) {
		if(_obj.pushFunction("prefetchTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(level);
			_obj.pushArg(tx);
			_obj.pushArg(ty);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::prefetchTile(level, tx, ty);
	};

	// void proland::GraphLayer::startCreateTile(int level, int tx, int ty, unsigned int deadline, ork::ptr< ork::Task > task, ork::ptr< ork::TaskGraph > owner)
	void startCreateTile(int level, int tx, int ty, unsigned int deadline, ork::ptr< ork::Task > task, ork::ptr< ork::TaskGraph > owner) {
		if(_obj.pushFunction("startCreateTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(level);
			_obj.pushArg(tx);
			_obj.pushArg(ty);
			_obj.pushArg(deadline);
			_obj.pushArg((ork::Task*)task.get());
			_obj.pushArg((ork::TaskGraph*)owner.get());
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::startCreateTile(level, tx, ty, deadline, task, owner);
	};

	// void proland::GraphLayer::beginCreateTile()
	void beginCreateTile() {
		if(_obj.pushFunction("beginCreateTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::beginCreateTile();
	};

	// void proland::GraphLayer::endCreateTile()
	void endCreateTile() {
		if(_obj.pushFunction("endCreateTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::endCreateTile();
	};

	// void proland::GraphLayer::stopCreateTile(int level, int tx, int ty)
	void stopCreateTile(int level, int tx, int ty) {
		if(_obj.pushFunction("stopCreateTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(level);
			_obj.pushArg(tx);
			_obj.pushArg(ty);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::stopCreateTile(level, tx, ty);
	};

	// void proland::MaskOrthoLayer::setTileSize(int tileSize, int tileBorder, float rootQuadSize)
	void setTileSize(int tileSize, int tileBorder, float rootQuadSize) {
		if(_obj.pushFunction("setTileSize")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(tileSize);
			_obj.pushArg(tileBorder);
			_obj.pushArg(rootQuadSize);
			return (_obj.callFunction<void>());
		}

		return MaskOrthoLayer::setTileSize(tileSize, tileBorder, rootQuadSize);
	};

	// bool proland::MaskOrthoLayer::doCreateTile(int level, int tx, int ty, proland::TileStorage::Slot * data)
	bool doCreateTile(int level, int tx, int ty, proland::TileStorage::Slot * data) {
		if(_obj.pushFunction("doCreateTile")) {
			_obj.pushArg((proland::MaskOrthoLayer*)this);
			_obj.pushArg(level);
			_obj.pushArg(tx);
			_obj.pushArg(ty);
			_obj.pushArg(data);
			return (_obj.callFunction<bool>());
		}

		return MaskOrthoLayer::doCreateTile(level, tx, ty, data);
	};


	// Protected non-virtual methods:
	// void proland::MaskOrthoLayer::init(ork::ptr< proland::GraphProducer > graphs, set< int > ignored, ork::ptr< ork::Program > layerProgram, int writeMask, ork::vec4f color, float depth, float widthFactor, proland::MaskOrthoLayer::BlendParams blendParams, ork::vec4f blendColor, int displayLevel, bool deform = false)
	void public_init(ork::ptr< proland::GraphProducer > graphs, set< int > ignored, ork::ptr< ork::Program > layerProgram, int writeMask, ork::vec4f color, float depth, float widthFactor, proland::MaskOrthoLayer::BlendParams blendParams, ork::vec4f blendColor, int displayLevel, bool deform = false) {
		return proland::MaskOrthoLayer::init(graphs, ignored, layerProgram, writeMask, color, depth, widthFactor, blendParams, blendColor, displayLevel, deform);
	};

	// void proland::MaskOrthoLayer::swap(ork::ptr< proland::MaskOrthoLayer > p)
	void public_swap(ork::ptr< proland::MaskOrthoLayer > p) {
		return proland::MaskOrthoLayer::swap(p);
	};


	// Protected non-virtual checkers:
	inline static bool _lg_typecheck_public_init(lua_State *L) {
		int luatop = lua_gettop(L);
		if( luatop<11 || luatop>12 ) return false;

		if( !Luna<void>::has_uniqueid(L,2,1381405) ) return false;
		if( !Luna< ork::Object >::checkSubType< proland::GraphProducer >(L,2) ) return false;
		if( !Luna<void>::has_uniqueid(L,3,13198241) ) return false;
		if( !Luna<void>::has_uniqueid(L,4,1381405) ) return false;
		if( !Luna< ork::Object >::checkSubType< ork::Program >(L,4) ) return false;
		if( (lua_type(L,5)!=LUA_TNUMBER || lua_tointeger(L,5) != lua_tonumber(L,5)) ) return false;
		if( !Luna<void>::has_uniqueid(L,6,81304302) ) return false;
		if( lua_type(L,7)!=LUA_TNUMBER ) return false;
		if( lua_type(L,8)!=LUA_TNUMBER ) return false;
		if( !Luna<void>::has_uniqueid(L,9,50727209) ) return false;
		if( !Luna<void>::has_uniqueid(L,10,81304302) ) return false;
		if( (lua_type(L,11)!=LUA_TNUMBER || lua_tointeger(L,11) != lua_tonumber(L,11)) ) return false;
		if( luatop>11 && lua_isboolean(L,12)==0 ) return false;
		return true;
	}

	inline static bool _lg_typecheck_public_swap(lua_State *L) {
		if( lua_gettop(L)!=2 ) return false;

		if( !Luna<void>::has_uniqueid(L,2,1381405) ) return false;
		if( !Luna< ork::Object >::checkSubType< proland::MaskOrthoLayer >(L,2) ) return false;
		return true;
	}


	// Protected non-virtual function binds:
	// void proland::MaskOrthoLayer::public_init(ork::ptr< proland::GraphProducer > graphs, set< int > ignored, ork::ptr< ork::Program > layerProgram, int writeMask, ork::vec4f color, float depth, float widthFactor, proland::MaskOrthoLayer::BlendParams blendParams, ork::vec4f blendColor, int displayLevel, bool deform = false)
	static int _bind_public_init(lua_State *L) {
		if (!_lg_typecheck_public_init(L)) {
			luaL_error(L, "luna typecheck failed in void proland::MaskOrthoLayer::public_init(ork::ptr< proland::GraphProducer > graphs, set< int > ignored, ork::ptr< ork::Program > layerProgram, int writeMask, ork::vec4f color, float depth, float widthFactor, proland::MaskOrthoLayer::BlendParams blendParams, ork::vec4f blendColor, int displayLevel, bool deform = false) function, expected prototype:\nvoid proland::MaskOrthoLayer::public_init(ork::ptr< proland::GraphProducer > graphs, set< int > ignored, ork::ptr< ork::Program > layerProgram, int writeMask, ork::vec4f color, float depth, float widthFactor, proland::MaskOrthoLayer::BlendParams blendParams, ork::vec4f blendColor, int displayLevel, bool deform = false)\nClass arguments details:\narg 1 ID = [unknown]\narg 2 ID = [unknown]\narg 3 ID = [unknown]\narg 5 ID = 81304302\narg 8 ID = 50727209\narg 9 ID = 81304302\n\n%s",luna_dumpStack(L).c_str());
		}

		int luatop = lua_gettop(L);

		ork::ptr< proland::GraphProducer > graphs = Luna< ork::Object >::checkSubType< proland::GraphProducer >(L,2);
		set< int >* ignored_ptr=(Luna< set< int > >::check(L,3));
		if( !ignored_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg ignored in proland::MaskOrthoLayer::public_init function");
		}
		set< int > ignored=*ignored_ptr;
		ork::ptr< ork::Program > layerProgram = Luna< ork::Object >::checkSubType< ork::Program >(L,4);
		int writeMask=(int)lua_tointeger(L,5);
		ork::vec4f* color_ptr=(Luna< ork::vec4f >::check(L,6));
		if( !color_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg color in proland::MaskOrthoLayer::public_init function");
		}
		ork::vec4f color=*color_ptr;
		float depth=(float)lua_tonumber(L,7);
		float widthFactor=(float)lua_tonumber(L,8);
		proland::MaskOrthoLayer::BlendParams* blendParams_ptr=(Luna< proland::MaskOrthoLayer::BlendParams >::check(L,9));
		if( !blendParams_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg blendParams in proland::MaskOrthoLayer::public_init function");
		}
		proland::MaskOrthoLayer::BlendParams blendParams=*blendParams_ptr;
		ork::vec4f* blendColor_ptr=(Luna< ork::vec4f >::check(L,10));
		if( !blendColor_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg blendColor in proland::MaskOrthoLayer::public_init function");
		}
		ork::vec4f blendColor=*blendColor_ptr;
		int displayLevel=(int)lua_tointeger(L,11);
		bool deform=luatop>11 ? (bool)(lua_toboolean(L,12)==1) : (bool)false;

		wrapper_proland_MaskOrthoLayer* self=Luna< ork::Object >::checkSubType< wrapper_proland_MaskOrthoLayer >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void proland::MaskOrthoLayer::public_init(ork::ptr< proland::GraphProducer >, set< int >, ork::ptr< ork::Program >, int, ork::vec4f, float, float, proland::MaskOrthoLayer::BlendParams, ork::vec4f, int, bool). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->public_init(graphs, ignored, layerProgram, writeMask, color, depth, widthFactor, blendParams, blendColor, displayLevel, deform);

		return 0;
	}

	// void proland::MaskOrthoLayer::public_swap(ork::ptr< proland::MaskOrthoLayer > p)
	static int _bind_public_swap(lua_State *L) {
		if (!_lg_typecheck_public_swap(L)) {
			luaL_error(L, "luna typecheck failed in void proland::MaskOrthoLayer::public_swap(ork::ptr< proland::MaskOrthoLayer > p) function, expected prototype:\nvoid proland::MaskOrthoLayer::public_swap(ork::ptr< proland::MaskOrthoLayer > p)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}

		ork::ptr< proland::MaskOrthoLayer > p = Luna< ork::Object >::checkSubType< proland::MaskOrthoLayer >(L,2);

		wrapper_proland_MaskOrthoLayer* self=Luna< ork::Object >::checkSubType< wrapper_proland_MaskOrthoLayer >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void proland::MaskOrthoLayer::public_swap(ork::ptr< proland::MaskOrthoLayer >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->public_swap(p);

		return 0;
	}


	void register_protected_methods(lua_State* L) {
		static const luaL_Reg wrapper_lib[] = {
		{"init",_bind_public_init},
		{"swap",_bind_public_swap},
		{NULL,NULL}
		};

		pushTable();
		luaL_register(L, NULL, wrapper_lib);
		lua_pop(L, 1);
	};


};




#endif

