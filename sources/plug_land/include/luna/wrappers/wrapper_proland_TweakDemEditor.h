#ifndef _WRAPPERS_WRAPPER_PROLAND_TWEAKDEMEDITOR_H_
#define _WRAPPERS_WRAPPER_PROLAND_TWEAKDEMEDITOR_H_
...
class wrapper_proland_TweakDemEditor : public proland::TweakDemEditor, public luna_wrapper_base {

public:
		

	~wrapper_proland_TweakDemEditor() {
		logDEBUG3("Calling delete function for wrapper proland_TweakDemEditor");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::TweakDemEditor*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_TweakDemEditor(lua_State* L, lua_Table* dum, bool active = true) 
		: proland::TweakDemEditor(active), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::TweakDemEditor*)this);
			_obj.callFunction<void>();
		}
	};
...
protected:
	...
	void doRelease() {
		...
		return TweakDemEditor::doRelease();
	};
	...
	void init(const char * name, ork::ptr< ork::EventHandler > eventHandler, bool active) {
		...
		return TweakDemEditor::init(name, eventHandler, active);
	};

	// void proland::TweakDemEditor::updateBar(TwBar * bar)
	void updateBar(TwBar * bar) {
		...
		return TweakDemEditor::updateBar(bar);
	};
...
public:
	...
	const char * toString() {
		...
		return TweakDemEditor::toString();
	};
	...
	void redisplay(double t, double dt, bool & needUpdate) {
		...
		return TweakDemEditor::redisplay(t, dt, needUpdate);
	};
	...
	void reshape(int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::reshape(x, y, needUpdate);
	};
	...
	void idle(bool damaged, bool & needUpdate) {
		...
		return TweakDemEditor::idle(damaged, needUpdate);
	};
	...
	bool mouseClick(ork::EventHandler::button b, ork::EventHandler::state s, ork::EventHandler::modifier m, int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::mouseClick(b, s, m, x, y, needUpdate);
	};
	...
	bool mouseWheel(ork::EventHandler::wheel b, ork::EventHandler::modifier m, int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::mouseWheel(b, m, x, y, needUpdate);
	};
	...
	bool mouseMotion(int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::mouseMotion(x, y, needUpdate);
	};
	...
	bool mousePassiveMotion(int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::mousePassiveMotion(x, y, needUpdate);
	};
	...
	bool keyTyped(unsigned char c, ork::EventHandler::modifier m, int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::keyTyped(c, m, x, y, needUpdate);
	};
	...
	bool keyReleased(unsigned char c, ork::EventHandler::modifier m, int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::keyReleased(c, m, x, y, needUpdate);
	};
	...
	bool specialKey(ork::EventHandler::key k, ork::EventHandler::modifier m, int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::specialKey(k, m, x, y, needUpdate);
	};
	...
	bool specialKeyReleased(ork::EventHandler::key k, ork::EventHandler::modifier m, int x, int y, bool & needUpdate) {
		...
		return TweakDemEditor::specialKeyReleased(k, m, x, y, needUpdate);
	};
	...
	void setActive(bool active) {
		...
		return TweakDemEditor::setActive(active);
	};
	...
	static int _bind_public_swap(lua_State *L) {
		...
		wrapper_proland_TweakDemEditor* self=Luna< ork::Object >::checkSubType< wrapper_proland_TweakDemEditor >(L,1);
		if(!self) {
