#ifndef _WRAPPERS_WRAPPER_PROLAND_LINECURVEPART_H_
#define _WRAPPERS_WRAPPER_PROLAND_LINECURVEPART_H_

#include <plug_common.h>

#include "lua/LuaObject.h"

#include <W:/Cloud/Projects/sgt/sources/proland/sources/graph/proland/graph/LineCurvePart.h>

class wrapper_proland_LineCurvePart : public proland::LineCurvePart, public luna_wrapper_base {

public:
		

	~wrapper_proland_LineCurvePart() {
		logDEBUG3("Calling delete function for wrapper proland_LineCurvePart");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::LineCurvePart*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_LineCurvePart(lua_State* L, lua_Table* dum, ork::vec2d start, ork::vec2d end) 
		: proland::LineCurvePart(start, end), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			_obj.callFunction<void>();
		}
	};


	// Private virtual methods:

	// Protected virtual methods:

	// Public virtual methods:
	// proland::CurveId proland::CurvePart::getId() const
	proland::CurveId getId() const {
		if(_obj.pushFunction("getId")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return *(_obj.callFunction<proland::CurveId*>());
		}

		return LineCurvePart::getId();
	};

	// proland::CurveId proland::CurvePart::getParentId() const
	proland::CurveId getParentId() const {
		if(_obj.pushFunction("getParentId")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return *(_obj.callFunction<proland::CurveId*>());
		}

		return LineCurvePart::getParentId();
	};

	// int proland::CurvePart::getType() const
	int getType() const {
		if(_obj.pushFunction("getType")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return (_obj.callFunction<int>());
		}

		return LineCurvePart::getType();
	};

	// float proland::CurvePart::getWidth() const
	float getWidth() const {
		if(_obj.pushFunction("getWidth")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return (_obj.callFunction<float>());
		}

		return LineCurvePart::getWidth();
	};

	// proland::CurvePtr proland::CurvePart::getCurve() const
	proland::CurvePtr getCurve() const {
		if(_obj.pushFunction("getCurve")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return *(_obj.callFunction<proland::CurvePtr*>());
		}

		return LineCurvePart::getCurve();
	};

	// bool proland::CurvePart::canClip(int i) const
	bool canClip(int i) const {
		if(_obj.pushFunction("canClip")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			_obj.pushArg(i);
			return (_obj.callFunction<bool>());
		}

		return LineCurvePart::canClip(i);
	};

	// int proland::LineCurvePart::getEnd() const
	int getEnd() const {
		if(_obj.pushFunction("getEnd")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return (_obj.callFunction<int>());
		}

		return LineCurvePart::getEnd();
	};

	// ork::vec2d proland::LineCurvePart::getXY(int i) const
	ork::vec2d getXY(int i) const {
		if(_obj.pushFunction("getXY")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			_obj.pushArg(i);
			return *(_obj.callFunction<ork::vec2d*>());
		}

		return LineCurvePart::getXY(i);
	};

	// bool proland::LineCurvePart::getIsControl(int i) const
	bool getIsControl(int i) const {
		if(_obj.pushFunction("getIsControl")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			_obj.pushArg(i);
			return (_obj.callFunction<bool>());
		}

		return LineCurvePart::getIsControl(i);
	};

	// float proland::LineCurvePart::getS(int i) const
	float getS(int i) const {
		if(_obj.pushFunction("getS")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			_obj.pushArg(i);
			return (_obj.callFunction<float>());
		}

		return LineCurvePart::getS(i);
	};

	// ork::box2d proland::LineCurvePart::getBounds() const
	ork::box2d getBounds() const {
		if(_obj.pushFunction("getBounds")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			return *(_obj.callFunction<ork::box2d*>());
		}

		return LineCurvePart::getBounds();
	};

	// proland::CurvePart * proland::LineCurvePart::clip(int start, int end) const
	proland::CurvePart * clip(int start, int end) const {
		if(_obj.pushFunction("clip")) {
			_obj.pushArg((proland::LineCurvePart*)this);
			_obj.pushArg(start);
			_obj.pushArg(end);
			return (_obj.callFunction<proland::CurvePart*>());
		}

		return LineCurvePart::clip(start, end);
	};


	// Protected non-virtual methods:

	// Protected non-virtual checkers:

	// Protected non-virtual function binds:

	void register_protected_methods(lua_State* L) {
	};


};




#endif

