#ifndef _WRAPPERS_WRAPPER_PROLAND_ROADELEVATIONLAYER_ROADELEVATIONCURVEDATA_H_
#define _WRAPPERS_WRAPPER_PROLAND_ROADELEVATIONLAYER_ROADELEVATIONCURVEDATA_H_
...
class wrapper_proland_RoadElevationLayer_RoadElevationCurveData : public proland::RoadElevationLayer::RoadElevationCurveData, public luna_wrapper_base {

public:
		

	~wrapper_proland_RoadElevationLayer_RoadElevationCurveData() {
		logDEBUG3("Calling delete function for wrapper proland_RoadElevationLayer_RoadElevationCurveData");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::RoadElevationLayer::RoadElevationCurveData*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_RoadElevationLayer_RoadElevationCurveData(lua_State* L, lua_Table* dum, proland::CurveId id, proland::CurvePtr flattenCurve, ork::ptr< proland::TileProducer > elevations) 
		: proland::RoadElevationLayer::RoadElevationCurveData(id, flattenCurve, elevations), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::RoadElevationLayer::RoadElevationCurveData*)this);
			_obj.callFunction<void>();
		}
	};
...
protected:
	...
	// float proland::RoadElevationLayer::RoadElevationCurveData::getCapLength(proland::NodePtr p, ork::vec2d q)
	float getCapLength(proland::NodePtr p, ork::vec2d q) {
		...
		return RoadElevationCurveData::getCapLength(p, q);
	};
...
public:
	...
	float getSampleLength(proland::CurvePtr c) {
		...
		return RoadElevationCurveData::getSampleLength(c);
	};
	...
	int getSmoothFactor(proland::CurvePtr c) {
		...
		return RoadElevationCurveData::getSmoothFactor(c);
	};
	...
	float getStartHeight() {
		...
		return RoadElevationCurveData::getStartHeight();
	};
	...
	float getEndHeight() {
		...
		return RoadElevationCurveData::getEndHeight();
	};
	...
	float getAltitude(float s) {
		...
		return RoadElevationCurveData::getAltitude(s);
	};
	...
	void getUsedTiles(set< proland::TileCache::Tile::Id > & tiles, float rootSampleLength) {
		...
		return RoadElevationCurveData::getUsedTiles(tiles, rootSampleLength);
	};
	...
	static int _bind_public_getSample(lua_State *L) {
		...
		wrapper_proland_RoadElevationLayer_RoadElevationCurveData* self=Luna< proland::CurveData >::checkSubType< wrapper_proland_RoadElevationLayer_RoadElevationCurveData >(L,1);
		if(!self) {
