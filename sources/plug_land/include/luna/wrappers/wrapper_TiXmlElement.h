#ifndef _WRAPPERS_WRAPPER_TIXMLELEMENT_H_
#define _WRAPPERS_WRAPPER_TIXMLELEMENT_H_
...
class wrapper_TiXmlElement : public TiXmlElement, public luna_wrapper_base {

public:
		

	~wrapper_TiXmlElement() {
		logDEBUG3("Calling delete function for wrapper TiXmlElement");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((TiXmlElement*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_TiXmlElement(lua_State* L, lua_Table* dum, const char * in_value) 
		: TiXmlElement(in_value), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlElement*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_TiXmlElement(lua_State* L, lua_Table* dum, const std::string & _value) 
		: TiXmlElement(_value), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlElement*)this);
			_obj.callFunction<void>();
	...
	wrapper_TiXmlElement(lua_State* L, lua_Table* dum, const TiXmlElement & arg1) 
		: TiXmlElement(arg1), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlElement*)this);
			_obj.callFunction<void>();
	...
	const TiXmlDocument * ToDocument() const {
		...
		return TiXmlElement::ToDocument();
	};
	...
	const TiXmlDeclaration * ToDeclaration() const {
		...
		return TiXmlElement::ToDeclaration();
	};
	...
	bool Accept(TiXmlVisitor * visitor) const {
		...
		return TiXmlElement::Accept(visitor);
	};


	// Protected non-virtual methods:
	// void TiXmlElement::CopyTo(TiXmlElement * target) const
	void public_CopyTo(TiXmlElement * target) const {
		return TiXmlElement::CopyTo(target);
	};

	// void TiXmlElement::ClearThis()
	void public_ClearThis() {
		return TiXmlElement::ClearThis();
	};
	...
	inline static bool _lg_typecheck_public_ClearThis(lua_State *L) {
	...
	// void TiXmlElement::public_CopyTo(TiXmlElement * target) const
	static int _bind_public_CopyTo(lua_State *L) {
		if (!_lg_typecheck_public_CopyTo(L)) {
			luaL_error(L, "luna typecheck failed in void TiXmlElement::public_CopyTo(TiXmlElement * target) const function, expected prototype:\nvoid TiXmlElement::public_CopyTo(TiXmlElement * target) const\nClass arguments details:\narg 1 ID = 89852901\n\n%s",luna_dumpStack(L).c_str());
		}

		TiXmlElement* target=(Luna< TiXmlBase >::checkSubType< TiXmlElement >(L,2));

		wrapper_TiXmlElement* self=Luna< TiXmlBase >::checkSubType< wrapper_TiXmlElement >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void TiXmlElement::public_CopyTo(TiXmlElement *) const. Got : '%s'\n%s",typeid(Luna< TiXmlBase >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// void TiXmlElement::public_ClearThis()
	static int _bind_public_ClearThis(lua_State *L) {
		if (!_lg_typecheck_public_ClearThis(L)) {
			luaL_error(L, "luna typecheck failed in void TiXmlElement::public_ClearThis() function, expected prototype:\nvoid TiXmlElement::public_ClearThis()\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		wrapper_TiXmlElement* self=Luna< TiXmlBase >::checkSubType< wrapper_TiXmlElement >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void TiXmlElement::public_ClearThis(). Got : '%s'\n%s",typeid(Luna< TiXmlBase >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->public_ClearThis();
	...
	void register_protected_methods(lua_State* L) {
		...
		{"ClearThis",_bind_public_ClearThis},
