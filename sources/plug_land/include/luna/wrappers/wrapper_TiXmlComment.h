#ifndef _WRAPPERS_WRAPPER_TIXMLCOMMENT_H_
#define _WRAPPERS_WRAPPER_TIXMLCOMMENT_H_
...
class wrapper_TiXmlComment : public TiXmlComment, public luna_wrapper_base {

public:
		

	~wrapper_TiXmlComment() {
		logDEBUG3("Calling delete function for wrapper TiXmlComment");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((TiXmlComment*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_TiXmlComment(lua_State* L, lua_Table* dum) 
		: TiXmlComment(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlComment*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_TiXmlComment(lua_State* L, lua_Table* dum, const char * _value) 
		: TiXmlComment(_value), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlComment*)this);
			_obj.callFunction<void>();
	...
	wrapper_TiXmlComment(lua_State* L, lua_Table* dum, const TiXmlComment & arg1) 
		: TiXmlComment(arg1), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlComment*)this);
			_obj.callFunction<void>();
	...
	const TiXmlDocument * ToDocument() const {
		...
		return TiXmlComment::ToDocument();
	};
	...
	const TiXmlDeclaration * ToDeclaration() const {
		...
		return TiXmlComment::ToDeclaration();
	};
	...
	bool Accept(TiXmlVisitor * visitor) const {
		...
		return TiXmlComment::Accept(visitor);
	};


	// Protected non-virtual methods:
	// void TiXmlComment::CopyTo(TiXmlComment * target) const
	void public_CopyTo(TiXmlComment * target) const {
		return TiXmlComment::CopyTo(target);
	};
	...
	// void TiXmlComment::public_CopyTo(TiXmlComment * target) const
	static int _bind_public_CopyTo(lua_State *L) {
		if (!_lg_typecheck_public_CopyTo(L)) {
			luaL_error(L, "luna typecheck failed in void TiXmlComment::public_CopyTo(TiXmlComment * target) const function, expected prototype:\nvoid TiXmlComment::public_CopyTo(TiXmlComment * target) const\nClass arguments details:\narg 1 ID = 89852901\n\n%s",luna_dumpStack(L).c_str());
		}

		TiXmlComment* target=(Luna< TiXmlBase >::checkSubType< TiXmlComment >(L,2));

		wrapper_TiXmlComment* self=Luna< TiXmlBase >::checkSubType< wrapper_TiXmlComment >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void TiXmlComment::public_CopyTo(TiXmlComment *) const. Got : '%s'\n%s",typeid(Luna< TiXmlBase >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
