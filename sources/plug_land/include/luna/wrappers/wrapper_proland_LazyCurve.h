#ifndef _WRAPPERS_WRAPPER_PROLAND_LAZYCURVE_H_
#define _WRAPPERS_WRAPPER_PROLAND_LAZYCURVE_H_
...
class wrapper_proland_LazyCurve : public proland::LazyCurve, public luna_wrapper_base {

public:
		

	~wrapper_proland_LazyCurve() {
		logDEBUG3("Calling delete function for wrapper proland_LazyCurve");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::LazyCurve*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_LazyCurve(lua_State* L, lua_Table* dum, proland::Graph * owner, proland::CurveId id) 
		: proland::LazyCurve(owner, id), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::LazyCurve*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_proland_LazyCurve(lua_State* L, lua_Table* dum, proland::Graph * owner, proland::CurveId id, proland::NodeId s, proland::NodeId e) 
		: proland::LazyCurve(owner, id, s, e), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::LazyCurve*)this);
			_obj.callFunction<void>();
...
private:
	...
	// void proland::LazyCurve::removeArea(proland::AreaId a)
	void removeArea(proland::AreaId a) {
		THROW_IF(!_obj.pushFunction("removeArea"),"No implementation for abstract function proland::LazyCurve::removeArea");
		_obj.pushArg((proland::LazyCurve*)this);
		_obj.pushArg(&a);
		return (_obj.callFunction<void>());
	};

	// void proland::LazyCurve::setParentId(proland::CurveId id)
	void setParentId(proland::CurveId id) {
		THROW_IF(!_obj.pushFunction("setParentId"),"No implementation for abstract function proland::LazyCurve::setParentId");
		_obj.pushArg((proland::LazyCurve*)this);
		_obj.pushArg(&id);
		return (_obj.callFunction<void>());
	};
...
protected:
	...
	// void proland::LazyCurve::doRelease()
	void doRelease() {
...
public:
	...
	proland::CurveId getAncestorId() const {
		...
		return LazyCurve::getAncestorId();
	};
	...
	void flatten(float squareFlatness) {
		...
		return LazyCurve::flatten(squareFlatness);
	};
	...
	void addVertex(proland::NodeId id, bool isEnd = 1) {
		...
		return LazyCurve::addVertex(id, isEnd);
	};
	...
	void loadVertex(proland::NodeId id, bool isEnd = 1) {
		...
		return LazyCurve::loadVertex(id, isEnd);
	};
	...
	void addVertex(double x, double y, float s, bool isControl) {
		...
		return LazyCurve::addVertex(x, y, s, isControl);
	};
	...
	void addVertex(ork::vec2d pt, int rank, bool isControl) {
		...
		return LazyCurve::addVertex(pt, rank, isControl);
	};
	...
	void addVertex(const ork::vec2d & p, float s, float l, bool isControl) {
		...
		return LazyCurve::addVertex(p, s, l, isControl);
	};
	...
	void addVertices(const vector< ork::vec2d > & v) {
		...
		return LazyCurve::addVertices(v);
	};
	...
	void removeVertex(int i) {
		...
		return LazyCurve::removeVertex(i);
	};
	...
	void setIsControl(int i, bool c) {
		...
		return LazyCurve::setIsControl(i, c);
	};
	...
	void setWidth(float width) {
		...
		return LazyCurve::setWidth(width);
	};
	...
	static int _bind_public_setParentId(lua_State *L) {
		...
		wrapper_proland_LazyCurve* self=Luna< ork::Object >::checkSubType< wrapper_proland_LazyCurve >(L,1);
		if(!self) {
