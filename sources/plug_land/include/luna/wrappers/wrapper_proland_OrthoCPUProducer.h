#ifndef _WRAPPERS_WRAPPER_PROLAND_ORTHOCPUPRODUCER_H_
#define _WRAPPERS_WRAPPER_PROLAND_ORTHOCPUPRODUCER_H_
...
class wrapper_proland_OrthoCPUProducer : public proland::OrthoCPUProducer, public luna_wrapper_base {

public:
		

	~wrapper_proland_OrthoCPUProducer() {
		logDEBUG3("Calling delete function for wrapper proland_OrthoCPUProducer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::OrthoCPUProducer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_OrthoCPUProducer(lua_State* L, lua_Table* dum, ork::ptr< proland::TileCache > cache, const char * name) 
		: proland::OrthoCPUProducer(cache, name), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::OrthoCPUProducer*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_proland_OrthoCPUProducer(lua_State* L, lua_Table* dum) 
		: proland::OrthoCPUProducer(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::OrthoCPUProducer*)this);
			_obj.callFunction<void>();
...
protected:
	...
	void doRelease() {
		...
		return OrthoCPUProducer::doRelease();
	};
	...
	void * getContext() const {
		...
		return OrthoCPUProducer::getContext();
	};
	...
	ork::ptr< ork::Task > startCreateTile(int level, int tx, int ty, unsigned int deadline, ork::ptr< ork::Task > task, ork::ptr< ork::TaskGraph > owner) {
		...
		return OrthoCPUProducer::startCreateTile(level, tx, ty, deadline, task, owner);
	};
	...
	void beginCreateTile() {
		...
		return OrthoCPUProducer::beginCreateTile();
	};
	...
	void endCreateTile() {
		...
		return OrthoCPUProducer::endCreateTile();
	};
	...
	void stopCreateTile(int level, int tx, int ty) {
		...
		return OrthoCPUProducer::stopCreateTile(level, tx, ty);
	};

	// void proland::OrthoCPUProducer::init(ork::ptr< proland::TileCache > cache, const char * name)
	void init(ork::ptr< proland::TileCache > cache, const char * name) {
		...
		return OrthoCPUProducer::init(cache, name);
	};

	// bool proland::OrthoCPUProducer::doCreateTile(int level, int tx, int ty, proland::TileStorage::Slot * data)
	...
	// void proland::OrthoCPUProducer::swap(ork::ptr< proland::OrthoCPUProducer > p)
	void swap(ork::ptr< proland::OrthoCPUProducer > p) {
		if(_obj.pushFunction("swap")) {
			_obj.pushArg((proland::OrthoCPUProducer*)this);
			_obj.pushArg((proland::OrthoCPUProducer*)p.get());
			return (_obj.callFunction<void>());
		}

		return OrthoCPUProducer::swap(p);
	};
...
public:
	...
	const char * toString() {
		...
		return OrthoCPUProducer::toString();
	};
	...
	void setRootQuadSize(float size) {
		...
		return OrthoCPUProducer::setRootQuadSize(size);
	};
	...
	ork::ptr< proland::TileCache > getCache() {
		...
		return OrthoCPUProducer::getCache();
	};
	...
	proland::TileCache::Tile * findTile(int level, int tx, int ty, bool includeCache = false, bool done = false) {
		...
		return OrthoCPUProducer::findTile(level, tx, ty, includeCache, done);
	};
	...
	proland::TileCache::Tile * getTile(int level, int tx, int ty, unsigned int deadline) {
		...
		return OrthoCPUProducer::getTile(level, tx, ty, deadline);
	};
	...
	bool prefetchTile(int level, int tx, int ty) {
		...
		return OrthoCPUProducer::prefetchTile(level, tx, ty);
	};
	...
	void putTile(proland::TileCache::Tile * t) {
		...
		return OrthoCPUProducer::putTile(t);
	};
	...
	void invalidateTiles() {
		...
		return OrthoCPUProducer::invalidateTiles();
	};
	...
	void invalidateTile(int level, int tx, int ty) {
		...
		return OrthoCPUProducer::invalidateTile(level, tx, ty);
	};
	...
	void update(ork::ptr< ork::SceneManager > scene) {
		...
		return OrthoCPUProducer::update(scene);
	};
	...
	void getReferencedProducers(std::vector< ork::ptr< proland::TileProducer > > & producers) const {
		...
		return OrthoCPUProducer::getReferencedProducers(producers);
	};
	...
	int getBorder() {
		...
		return OrthoCPUProducer::getBorder();
	};
	...
	bool hasTile(int level, int tx, int ty) {
		...
		return OrthoCPUProducer::hasTile(level, tx, ty);
	};
	...
	static int _bind_public_init(lua_State *L) {
		...
		wrapper_proland_OrthoCPUProducer* self=Luna< ork::Object >::checkSubType< wrapper_proland_OrthoCPUProducer >(L,1);
		if(!self) {
