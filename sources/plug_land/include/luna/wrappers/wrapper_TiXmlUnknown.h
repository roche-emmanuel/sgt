#ifndef _WRAPPERS_WRAPPER_TIXMLUNKNOWN_H_
#define _WRAPPERS_WRAPPER_TIXMLUNKNOWN_H_
...
class wrapper_TiXmlUnknown : public TiXmlUnknown, public luna_wrapper_base {

public:
		

	~wrapper_TiXmlUnknown() {
		logDEBUG3("Calling delete function for wrapper TiXmlUnknown");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((TiXmlUnknown*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_TiXmlUnknown(lua_State* L, lua_Table* dum) 
		: TiXmlUnknown(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlUnknown*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_TiXmlUnknown(lua_State* L, lua_Table* dum, const TiXmlUnknown & copy) 
		: TiXmlUnknown(copy), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlUnknown*)this);
			_obj.callFunction<void>();
	...
	const TiXmlDocument * ToDocument() const {
		...
		return TiXmlUnknown::ToDocument();
	};
	...
	const TiXmlDeclaration * ToDeclaration() const {
		...
		return TiXmlUnknown::ToDeclaration();
	};
	...
	bool Accept(TiXmlVisitor * content) const {
		...
		return TiXmlUnknown::Accept(content);
	};


	// Protected non-virtual methods:
	// void TiXmlUnknown::CopyTo(TiXmlUnknown * target) const
	void public_CopyTo(TiXmlUnknown * target) const {
		return TiXmlUnknown::CopyTo(target);
	};
	...
	// void TiXmlUnknown::public_CopyTo(TiXmlUnknown * target) const
	static int _bind_public_CopyTo(lua_State *L) {
		if (!_lg_typecheck_public_CopyTo(L)) {
			luaL_error(L, "luna typecheck failed in void TiXmlUnknown::public_CopyTo(TiXmlUnknown * target) const function, expected prototype:\nvoid TiXmlUnknown::public_CopyTo(TiXmlUnknown * target) const\nClass arguments details:\narg 1 ID = 89852901\n\n%s",luna_dumpStack(L).c_str());
		}

		TiXmlUnknown* target=(Luna< TiXmlBase >::checkSubType< TiXmlUnknown >(L,2));

		wrapper_TiXmlUnknown* self=Luna< TiXmlBase >::checkSubType< wrapper_TiXmlUnknown >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void TiXmlUnknown::public_CopyTo(TiXmlUnknown *) const. Got : '%s'\n%s",typeid(Luna< TiXmlBase >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
