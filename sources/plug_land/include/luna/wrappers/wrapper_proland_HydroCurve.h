#ifndef _WRAPPERS_WRAPPER_PROLAND_HYDROCURVE_H_
#define _WRAPPERS_WRAPPER_PROLAND_HYDROCURVE_H_
...
class wrapper_proland_HydroCurve : public proland::HydroCurve, public luna_wrapper_base {

public:
		

	~wrapper_proland_HydroCurve() {
		logDEBUG3("Calling delete function for wrapper proland_HydroCurve");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::HydroCurve*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_HydroCurve(lua_State* L, lua_Table* dum, proland::Graph * owner) 
		: proland::HydroCurve(owner), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::HydroCurve*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_proland_HydroCurve(lua_State* L, lua_Table* dum, proland::Graph * owner, proland::CurvePtr c, proland::NodePtr s, proland::NodePtr e) 
		: proland::HydroCurve(owner, c, s, e), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::HydroCurve*)this);
			_obj.callFunction<void>();
...
public:
	...
	proland::CurveId getAncestorId() const {
		...
		return HydroCurve::getAncestorId();
	};
	...
	void addVertex(proland::NodeId id, bool isEnd = 1) {
		...
		return HydroCurve::addVertex(id, isEnd);
	};
	...
	void addVertex(double x, double y, float s, bool isControl) {
		...
		return HydroCurve::addVertex(x, y, s, isControl);
	};
	...
	void addVertex(ork::vec2d pt, int rank, bool isControl) {
		...
		return HydroCurve::addVertex(pt, rank, isControl);
	};
	...
	void addVertex(const ork::vec2d & p, float s, float l, bool isControl) {
		...
		return HydroCurve::addVertex(p, s, l, isControl);
	};
	...
	void addVertex(const proland::Vertex & pt) {
		...
		return HydroCurve::addVertex(pt);
	};
	...
	void addVertices(const vector< ork::vec2d > & v) {
		...
		return HydroCurve::addVertices(v);
	};
	...
	void removeVertex(int i) {
		...
		return HydroCurve::removeVertex(i);
	};
	...
	void setIsControl(int i, bool c) {
		...
		return HydroCurve::setIsControl(i, c);
	};
	...
	void setWidth(float width) {
		...
		return HydroCurve::setWidth(width);
	};
	...
	void setType(int type) {
		...
		return HydroCurve::setType(type);
	};
	...
	void flatten(float squareFlatness) {
		...
		return HydroCurve::flatten(squareFlatness);
	};
	...
	static int _bind_public_setParentId(lua_State *L) {
		...
		wrapper_proland_HydroCurve* self=Luna< ork::Object >::checkSubType< wrapper_proland_HydroCurve >(L,1);
		if(!self) {
