#ifndef _WRAPPERS_WRAPPER_ORK_TEXTURE_DMULTISAMPLEARRAY_H_
#define _WRAPPERS_WRAPPER_ORK_TEXTURE_DMULTISAMPLEARRAY_H_
...
class wrapper_ork_Texture2DMultisampleArray : public ork::Texture2DMultisampleArray, public luna_wrapper_base {

public:
		

	~wrapper_ork_Texture2DMultisampleArray() {
		logDEBUG3("Calling delete function for wrapper ork_Texture2DMultisampleArray");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((ork::Texture2DMultisampleArray*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_ork_Texture2DMultisampleArray(lua_State* L, lua_Table* dum, int w, int h, int l, int samples, ork::TextureInternalFormat tf, bool fixedLocations) 
		: ork::Texture2DMultisampleArray(w, h, l, samples, tf, fixedLocations), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((ork::Texture2DMultisampleArray*)this);
			_obj.callFunction<void>();
		}
	};
...
protected:
	...
	void doRelease() {
		...
		return Texture2DMultisampleArray::doRelease();
	};
	...
	void swap(ork::ptr< ork::Texture > t) {
		...
		return Texture2DMultisampleArray::swap(t);
	};
...
public:
	...
	const char * toString() {
		...
		return Texture2DMultisampleArray::toString();
	};
	...
	static int _bind_public_init(lua_State *L) {
		...
		wrapper_ork_Texture2DMultisampleArray* self=Luna< ork::Object >::checkSubType< wrapper_ork_Texture2DMultisampleArray >(L,1);
		if(!self) {
