#ifndef _WRAPPERS_WRAPPER_TIXMLDOCUMENT_H_
#define _WRAPPERS_WRAPPER_TIXMLDOCUMENT_H_
...
class wrapper_TiXmlDocument : public TiXmlDocument, public luna_wrapper_base {

public:
		

	~wrapper_TiXmlDocument() {
		logDEBUG3("Calling delete function for wrapper TiXmlDocument");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((TiXmlDocument*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_TiXmlDocument(lua_State* L, lua_Table* dum) 
		: TiXmlDocument(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlDocument*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_TiXmlDocument(lua_State* L, lua_Table* dum, const char * documentName) 
		: TiXmlDocument(documentName), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlDocument*)this);
			_obj.callFunction<void>();
	...
	wrapper_TiXmlDocument(lua_State* L, lua_Table* dum, const std::string & documentName) 
		: TiXmlDocument(documentName), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlDocument*)this);
	...
	wrapper_TiXmlDocument(lua_State* L, lua_Table* dum, const TiXmlDocument & copy) 
		: TiXmlDocument(copy), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((TiXmlDocument*)this);
			_obj.callFunction<void>();
...
public:
	...
	const TiXmlElement * ToElement() const {
		...
		return TiXmlDocument::ToElement();
	};
	...
	const TiXmlComment * ToComment() const {
		...
		return TiXmlDocument::ToComment();
	};
	...
	const TiXmlUnknown * ToUnknown() const {
		...
		return TiXmlDocument::ToUnknown();
	};
	...
	const TiXmlDeclaration * ToDeclaration() const {
		...
		return TiXmlDocument::ToDeclaration();
	};
	...
	const TiXmlDocument * ToDocument() const {
		...
		return TiXmlDocument::ToDocument();
	};
	...
	bool Accept(TiXmlVisitor * content) const {
		...
		return TiXmlDocument::Accept(content);
	};
	...
	static int _bind_public_CopyTo(lua_State *L) {
		...
		wrapper_TiXmlDocument* self=Luna< TiXmlBase >::checkSubType< wrapper_TiXmlDocument >(L,1);
		if(!self) {
