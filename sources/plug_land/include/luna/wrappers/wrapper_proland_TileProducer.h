#ifndef _WRAPPERS_WRAPPER_PROLAND_TILEPRODUCER_H_
#define _WRAPPERS_WRAPPER_PROLAND_TILEPRODUCER_H_
...
public:
	...
	~wrapper_proland_TileProducer() {
		logDEBUG3("Calling delete function for wrapper proland_TileProducer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((proland::TileProducer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_proland_TileProducer(lua_State* L, lua_Table* dum, const char * type, const char * taskType, ork::ptr< proland::TileCache > cache, bool gpuProducer) 
		: proland::TileProducer(type, taskType, cache, gpuProducer), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::TileProducer*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_proland_TileProducer(lua_State* L, lua_Table* dum, const char * type, const char * taskType) 
		: proland::TileProducer(type, taskType), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((proland::TileProducer*)this);
			_obj.callFunction<void>();
...
protected:
	...
	// bool proland::TileProducer::doCreateTile(int level, int tx, int ty, proland::TileStorage::Slot * data)
...
public:
	...
	void update(ork::ptr< ork::SceneManager > scene) {
		...
		return TileProducer::update(scene);
	};
	...
	static int _bind_public_init(lua_State *L) {
		...
		wrapper_proland_TileProducer* self=Luna< ork::Object >::checkSubType< wrapper_proland_TileProducer >(L,1);
		if(!self) {
