#ifndef _WRAPPERS_WRAPPER_ORK_TEXTUREBUFFER_H_
#define _WRAPPERS_WRAPPER_ORK_TEXTUREBUFFER_H_
...
class wrapper_ork_TextureBuffer : public ork::TextureBuffer, public luna_wrapper_base {

public:
		

	~wrapper_ork_TextureBuffer() {
		logDEBUG3("Calling delete function for wrapper ork_TextureBuffer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((ork::TextureBuffer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_ork_TextureBuffer(lua_State* L, lua_Table* dum, ork::TextureInternalFormat tf, ork::ptr< ork::GPUBuffer > pixels) 
		: ork::TextureBuffer(tf, pixels), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((ork::TextureBuffer*)this);
			_obj.callFunction<void>();
		}
	};
...
protected:
	...
	void doRelease() {
		...
		return TextureBuffer::doRelease();
	};
...
public:
	...
	static int _bind_public_init(lua_State *L) {
		...
		wrapper_ork_TextureBuffer* self=Luna< ork::Object >::checkSubType< wrapper_ork_TextureBuffer >(L,1);
		if(!self) {
