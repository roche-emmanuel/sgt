...
public:
	...
	// bool wxTreebook::base_IsDoubleBuffered() const
	static int _bind_base_IsDoubleBuffered(lua_State *L) {
		if (!_lg_typecheck_base_IsDoubleBuffered(L)) {
			luaL_error(L, "luna typecheck failed in bool wxTreebook::base_IsDoubleBuffered() const function, expected prototype:\nbool wxTreebook::base_IsDoubleBuffered() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		wxTreebook* self=Luna< wxObject >::checkSubType< wxTreebook >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call bool wxTreebook::base_IsDoubleBuffered() const. Got : '%s'\n%s",typeid(Luna< wxObject >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		bool lret = self->wxTreebook::IsDoubleBuffered();
		lua_pushboolean(L,lret?1:0);
...
luna_RegType LunaTraits< wxTreebook >::methods[] = {
	...
	{"base_IsDoubleBuffered", &luna_wrapper_wxTreebook::_bind_base_IsDoubleBuffered},
