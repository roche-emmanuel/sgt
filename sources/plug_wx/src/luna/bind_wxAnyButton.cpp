...
public:
	...
	// bool wxAnyButton::base_IsDoubleBuffered() const
	static int _bind_base_IsDoubleBuffered(lua_State *L) {
		if (!_lg_typecheck_base_IsDoubleBuffered(L)) {
			luaL_error(L, "luna typecheck failed in bool wxAnyButton::base_IsDoubleBuffered() const function, expected prototype:\nbool wxAnyButton::base_IsDoubleBuffered() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		wxAnyButton* self=Luna< wxObject >::checkSubType< wxAnyButton >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call bool wxAnyButton::base_IsDoubleBuffered() const. Got : '%s'\n%s",typeid(Luna< wxObject >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		bool lret = self->wxAnyButton::IsDoubleBuffered();
		lua_pushboolean(L,lret?1:0);
...
luna_RegType LunaTraits< wxAnyButton >::methods[] = {
	...
	{"base_IsDoubleBuffered", &luna_wrapper_wxAnyButton::_bind_base_IsDoubleBuffered},
