...
public:
	...
	// bool wxPreviewFrame::base_IsDoubleBuffered() const
	static int _bind_base_IsDoubleBuffered(lua_State *L) {
		if (!_lg_typecheck_base_IsDoubleBuffered(L)) {
			luaL_error(L, "luna typecheck failed in bool wxPreviewFrame::base_IsDoubleBuffered() const function, expected prototype:\nbool wxPreviewFrame::base_IsDoubleBuffered() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		wxPreviewFrame* self=Luna< wxObject >::checkSubType< wxPreviewFrame >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call bool wxPreviewFrame::base_IsDoubleBuffered() const. Got : '%s'\n%s",typeid(Luna< wxObject >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		bool lret = self->wxPreviewFrame::IsDoubleBuffered();
		lua_pushboolean(L,lret?1:0);
...
luna_RegType LunaTraits< wxPreviewFrame >::methods[] = {
	...
	{"base_IsDoubleBuffered", &luna_wrapper_wxPreviewFrame::_bind_base_IsDoubleBuffered},
