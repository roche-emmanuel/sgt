...
public:
	...
	// bool wxGauge::base_IsDoubleBuffered() const
	static int _bind_base_IsDoubleBuffered(lua_State *L) {
		if (!_lg_typecheck_base_IsDoubleBuffered(L)) {
			luaL_error(L, "luna typecheck failed in bool wxGauge::base_IsDoubleBuffered() const function, expected prototype:\nbool wxGauge::base_IsDoubleBuffered() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		wxGauge* self=Luna< wxObject >::checkSubType< wxGauge >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call bool wxGauge::base_IsDoubleBuffered() const. Got : '%s'\n%s",typeid(Luna< wxObject >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		bool lret = self->wxGauge::IsDoubleBuffered();
		lua_pushboolean(L,lret?1:0);
...
luna_RegType LunaTraits< wxGauge >::methods[] = {
	...
	{"base_IsDoubleBuffered", &luna_wrapper_wxGauge::_bind_base_IsDoubleBuffered},
