log:notice "Starting Proland terrain preprocess test."
...
function Class:initialize()
  ...
  local path = root_path.."/proland_samples/terrain/preprocess"
  -- self:preprocessAtmo(path,gw)

  self:debug("Generating DEMs...")

  local srtm = path.."/srtm.png"
  local input = land.SimpleInputMap(srtm,10000.0 / 255.0,1350, 675, 4, 225)

  proland.preprocessSphericalDem(input,24, 192, 2, path.."/data/dem", path.."/temp/tmpDem", 1.0)

  self:debug("Generating Ortho...")

  local ortho = path.."/bluemarble.png"
  local input = land.SimpleInputMap(ortho,1.0,1350, 675, 4, 225)

  proland.preprocessSphericalOrtho(input, 192, 4, 2, path.."/data/rgb", path.."/temp/tmpOrtho")
