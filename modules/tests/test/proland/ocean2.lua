log:notice "Starting Proland terrain ocean2 test."
...
function Class:preprocessAtmo(destPath,context)
  self:debug("Preprocessing atmosphere for path: ",destPath)
  
  -- before generating the images we should check they are already on the disk:
  local path = require "utils.path"
  if path.isFile(destPath.."/inscatter.raw") then
    self:debug("No need to generate atmosphere files.")
    return;
  end

  local params = proland.AtmoParameters()
  -- proland.preprocessAtmo(params,destPath)

  local processor = proland.AtmoProcessor(params, destPath)
  while(not processor:preprocess()) do
    -- self:debug("Preprocessing atmosphere...")
    context:swapBuffers()
  end
  self:debug("Done generating atmosphere!")
end
...
function Class:initialize()
  ...
  local path = root_path.."/proland_samples/ocean/ocean2"
  ...
  resLoader:addPath(root_path.."/proland_samples/ocean/ocean1"); -- this one also depends on ocean1 files.
  ...
  self._controller = proland.PlanetViewController(manager:getCameraNode(), 6360000.0);
  ...
  self._viewManager = proland.ViewManager{
    ...
    getWorldCoordinates = function(obj, tt, x, y)
      ...
      if p:length() > self._controller:getR()*1.1 then
  ...
  self._ui = resManager:loadResource("tweakBar"):dynCast("proland::TweakBarManager")
